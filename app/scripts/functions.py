    ...
    if backend == "mtcnn":
        ...
        if boxes is not None:
            for box, prob in zip(boxes, probs):
                ...
                results.append({"box": [max(0, x1), max(0, y1), width, height], "confidence": prob})
        return results
...
def get_embedding(model, device, face_rgb):
    ...
    face_tensor = (
        torch.from_numpy(face_resized).permute(2, 0, 1).float().to(device)
    )
    face_tensor = (face_tensor / 255.0 - 0.5) / 0.5

    with torch.no_grad():
        emb = model(face_tensor.unsqueeze(0))
    return np.asarray(emb.cpu().numpy().reshape(-1))
